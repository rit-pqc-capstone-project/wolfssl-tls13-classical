# wolfSSL TLS 1.3 — Classical Implementation

A TLS 1.3 server and client using wolfSSL with classical (ECDHE) key exchange.
...
- **wolfSSL source** cloned from GitHub
...
2. In the **wolfssl** project properties (not the whole solution), go to **C/C++ → Preprocessor → Preprocessor Definitions** and add `WOLFSSL_USER_SETTINGS` (keep any existing defines).
3. If wolfSSL cannot find `user_settings.h`, you may need to edit `wolfssl/wolfcrypt/settings.h` and change the `#include "user_settings.h"` line to use an absolute path:
   ```c
   #include "C:/Users/<you>/source/repos/wolfssl/wolfssl/user_settings.h"
   ```
4. Build the **wolfssl** project only (right-click → Build). Note the output path of `wolfssl.lib` (typically `Debug\x64\wolfssl.lib`).
...
2. Name it `tls13-classical-server`. Set the solution name to `tls13-classical`.
3. Right-click the Solution → Add → New Project → **Empty Project**, name it `tls13-classical-client`.
...
2. Run the server first: right-click `tls13-classical-server` → Set as Startup Project → Ctrl+F5.
3. Run the client: right-click `tls13-classical-client` → Set as Startup Project → Ctrl+F5.
